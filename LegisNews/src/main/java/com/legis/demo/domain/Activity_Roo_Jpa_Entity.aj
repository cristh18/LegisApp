// WARNING: DO NOT EDIT THIS FILE. THIS FILE IS MANAGED BY SPRING ROO.
// You may push code into the target .java compilation unit if you wish to edit any member(s).

package com.legis.demo.domain;

import com.legis.demo.domain.Activity;
import javax.persistence.Column;
import javax.persistence.DiscriminatorColumn;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.Inheritance;
import javax.persistence.InheritanceType;
import javax.persistence.Version;

privileged aspect Activity_Roo_Jpa_Entity {
    
    declare @type: Activity: @Entity;
    
    declare @type: Activity: @Inheritance(strategy = InheritanceType.SINGLE_TABLE);
    
    declare @type: Activity: @DiscriminatorColumn;
    
    @Id
    @GeneratedValue(strategy = GenerationType.AUTO)
    @Column(name = "id_")
    private Long Activity.id_;
    
    @Version
    @Column(name = "version")
    private Integer Activity.version;
    
    public Long Activity.getId_() {
        return this.id_;
    }
    
    public void Activity.setId_(Long id) {
        this.id_ = id;
    }
    
    public Integer Activity.getVersion() {
        return this.version;
    }
    
    public void Activity.setVersion(Integer version) {
        this.version = version;
    }
    
}
